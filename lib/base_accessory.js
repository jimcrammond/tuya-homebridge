...
class BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig, categoryType, serviceType, subServices = []) {
    ...
    if (this.homebridgeAccessory) {
      ...
      this.log.debug(`Existing Accessory found ${homebridgeAccessory.displayName}  ${homebridgeAccessory.context.deviceId} ${homebridgeAccessory.UUID}`);
      this.homebridgeAccessory.displayName = this.deviceConfig.name;
    }
    else {
      // Create new Accessory
      this.log.debug(`Creating New Accessory ${this.deviceConfig.id}`);
